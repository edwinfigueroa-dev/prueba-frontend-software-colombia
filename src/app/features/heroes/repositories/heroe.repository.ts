...
export class HeroeRepository implements IHeroeRepository {
  ...
  constructor(
    @Inject(HTTP_CLIENT) private _httpClient: IHttpClient
  ) {}

  getAll({ size, page }: { size: number, page: number }): Observable<Heroe[]> {
    return this._httpClient.get<Heroe[]>(`${environment.API_URL}/heroes?size=${size}&page=${page}`);
  }
